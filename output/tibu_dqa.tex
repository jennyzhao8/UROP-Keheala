...
\begin{itemize}
\itemsep0pt
    \item \textbf{Type~I error} (false positive in the digital record): TIBU records a successful outcome (treatment completion or cure) but the paper registry records a failure (death, treatment failure, or loss to follow-up). Clinically: TIBU \emph{overstates} the program's success.
    \item \textbf{Type~II error} (false negative in the digital record): TIBU records a failure while the paper registry records success. Clinically: TIBU \emph{understates} the program's success.
...
\end{itemize}
The cross-tabulation in Table~\ref{tbl:crosstab} maps the full distribution of these discrepancies across outcome categories; subsequent tables stratify error rates by clinic characteristics (province, urbanicity, and size) and patient characteristics (sex, age, disease type, and HIV status); and the smoothed time-series figures trace how error rates vary across the study period and with the age of the patient record.

\vspace{6pt}
\noindent\fbox{\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule}
\vspace{2pt}
\textbf{Key findings.} Across 3,054 classifiable audit records: Type~I (false positive in TIBU) raw rate 1.0\%; Type~II (false negative) rate 0.7\%; records missing from TIBU 3.8\%. The Type~I rate likely overstates true TIBU misclassification: $\sim 2/3$ of Type~I records appear to be audit-snapshot artifacts (paper was updated after the audit transcribed it; TIBU then reflected the revised outcome), so the residual Type~I rate is closer to $\sim 0.3\%$ -- see \S\ref{sec:date_errors}. Type~II and missing-in-TIBU rates are not affected by this caveat. The missing-in-TIBU rate is dominated by a COVID-era reporting gap ($\sim$15\% for treatment outcomes in 2020 Q1--Q2), concentrated at two clinics; outside that window, all three outcome-error rates are low and roughly flat across the study period.
\vspace{2pt}
\end{minipage}}
\vspace{6pt}
...
matched patients ($N = 3{,}370$). Counts on the diagonal are concordant; off-diagonal cells are the discrepancies that drive the error rates reported in the subsequent tables. The shaded regions mark the Type~I (red), Type~II (green), and missing-in-TIBU (blue) partitions used throughout the document.
...
Table~\ref{tbl:clinic_errors} reports false-positive, false-negative, and missing-in-TIBU rates stratified by province, urbanicity, and clinic size.
...
missing-in-TIBU rates broken down by individual characteristics and disease
characteristics.
...
\noindent Type~II errors have a notably longer reporting lag than no-error records (median $1{,}026$ vs.\ $850$ days) but a nearly identical record age (median $1{,}040$ vs.\ $1{,}004$ days). The asymmetry is informative. Reporting lag is measured from the \emph{TIBU} outcome date, which for a Type~II record is the early failure determination TIBU made (e.g., LTFU declared at month~6) rather than the later completion the patient actually reached: median TIBU outcome dates for Type~II records are roughly five months earlier than for no-error records, while median \emph{paper} outcome dates are only two months earlier. Record age, which depends only on registration date, shows no such gap---registration dates for both categories are drawn from the same April 2018--December 2019 window.

\noindent Stratifying instead by paper outcome date (the ground-truth timestamp) shows a Type~II rate that is roughly flat across the study period (0.4--1.5\% per quarter from 2018 Q3 through 2020 Q2), and the 22 Type~II records are spread across 12 clinics with no overlap with the two clinics responsible for the COVID-era missing-in-TIBU spike. On this reading, the apparent concentration of Type~II errors at long reporting lags is largely an artifact of where TIBU's (incorrect) outcome date lands on the calendar---an early ``failed'' closure is mechanically older than a later ``completed'' closure would have been---rather than evidence of a temporal gradient in the underlying error rate.
...
\noindent \textit{Sensitivity check.} Figure~\ref{fig:over_time_paper} replays the same analysis using the paper-registry outcome date throughout (rather than TIBU date with paper fallback for missing-in-TIBU records). Records with no parseable paper date are excluded ($n = $ \textasciitilde{}13\%). The COVID-era spike in missing-in-TIBU remains clearly visible, confirming that it is driven by when outcomes actually occurred (as captured on paper), not by when TIBU happened to record them.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{fig_error_over_time_paper.pdf}
    \caption{Sensitivity check: DQA error rates over time using paper-registry outcome dates throughout. Records without a parseable paper date are excluded. Compare with Figure~\ref{fig:over_time}.}
    \label{fig:over_time_paper}
\end{figure}

\noindent The dominant feature of the figure is a pronounced peak in the missing-in-TIBU rate (to roughly 15\%) for records with outcome dates in 2020 Q1--Q2, coinciding with the onset of the COVID-19 pandemic response in Kenya. Of the 116 missing-in-TIBU records in the classifiable sample, 63 fall in these two quarters, and 50 are concentrated at just two clinics whose overall missing-in-TIBU rates (12.7\% and 15.4\%) are an order of magnitude above the study-wide rate of 3.8\%. At these clinics, paper-registry entries continued through the pandemic but TIBU outcomes were largely not recorded. \texttt{NC} is a TIBU interim code denoting that treatment is still in progress; treating it as equivalent to blank (rather than as a medical failure) rolls the affected records into the missing-in-TIBU category, which accounts for 76 of the 116 (the remaining 40 have TIBU entirely blank). Type~I and Type~II error rates are low and roughly flat across the study period.
...
\begin{figure}[H]
    ...
    \caption{TIBU outcome date vs.\ paper outcome date for DQA-matched records with parseable dates on both sides (axes clipped to the 2018--2022 study window). Points on the $y=x$ dashed line agree exactly. Type~I points (red) sit predominantly above the line (TIBU later), Type~II points (green) predominantly below (TIBU earlier). The multiple parallel 45-degree lines are because it is common for TIBU and the paper audit to disagree on the year of the record.}
    \label{fig:tibu_vs_paper_date}
\end{figure}

Among no-error records the two dates agree to within a week $\sim 78\%$ of the time, with a small symmetric tail. The pattern is qualitatively different for the two error classes: for Type~I records, TIBU is \emph{later} than the audit-transcribed paper date in $16$ of $24$ cases ($\sim 2/3$); for Type~II records, TIBU is \emph{earlier} in $14$ of $19$ ($\sim 3/4$).  This suggests that about two-thirds of Type I errors are not in fact errors, but result from the paper audit occasionally having employed less up-to-date records than TIBU.
...
\iffalse
 ...
 but did not treat them as a priority. The paper-date column in the audit therefore cannot be treated as ground truth for date accuracy in the way that paper outcomes can. We consequently keep this section narrow, focused on what the date comparison reveals about the outcome-error counts in \S\ref{sec:outcome_errors} -- in particular, that many of the apparent Type~I records are not TIBU errors at all.
...
\begin{table}[H]
    ...
    \input{tblSI_date_consistency.tex}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{fig_tibu_vs_paper_date.pdf}
    \caption{TIBU outcome date vs.\ paper outcome date for DQA-matched records with parseable dates on both sides (axes clipped to the 2018--2022 study window). Points on the $y=x$ dashed line agree exactly; the faint dotted lines mark $y = x \pm 1$ and $\pm 2$ years. Type~I points (red) sit predominantly above the $y=x$ line (TIBU later), Type~II points (green) predominantly below (TIBU earlier).}
    \label{fig:tibu_vs_paper_date}
\end{figure}

The Type~I pattern is the signature of an audit-snapshot artifact rather than a TIBU error. Each clinic's paper register continues to be updated as outcomes accrue; for a patient marked LTFU at month~3 on paper, then transcribed by the audit, who later returns and completes treatment, TIBU eventually reflects the completed outcome while the audit retains the earlier interim one. The TIBU outcome date in such a case lands \emph{after} the paper outcome date the audit captured -- exactly what we see in $\sim 2/3$ of Type~I records. The headline Type~I rate of $1.0\%$ in \S\ref{sec:outcome_errors} therefore likely overstates true TIBU misclassification; the residual is closer to $\sim 0.3\%$. By contrast, the Type~II and missing-in-TIBU rates are not affected by this caveat: paper at audit time clearly held a later outcome than TIBU's earlier closure (Type~II), or paper had an outcome that TIBU never recorded at all (missing-in-TIBU). Both look like real TIBU lapses.
